...
output_file_path = 'htdocs/admin/output2.php'  # Path untuk upload output HTML
...
def save_output_to_php(results):
    with open('output.php', 'w') as f:
        ...
        for domain, status in results:
            f.write(f"$status2[{status_counter}] = '{status}';\n")  # Simpan status dengan penambahan angka
...
def main():
    ...
    upload_to_ftp('output2.php', output_file_path)  # Upload file output.php ke FTP
